...
Object.assign(App,{
  ...
  collect(){ const self=this,out=[]; this.state.nodes.forEach(n=>{ if(!n.line)return; const r=self.resMap[n.id]; if(!r||!r.res)return; out.push({n,line:n.line,res:r.res,err:r.error}); }); return out; },
  ...
  bindReport(){ const self=this, host=self.$("#reportForm");
    if(host){ host.innerHTML="";
      ...
      const base=self.el("input",{type:"text",value:"3300-E-000-EL-PHI-00009-00-D ред.03U (Ямал СПГ); ПУЭ изд.7 гл.1.3/3.1/7.3; ГОСТ 32144-2013; ГОСТ 31996-2012; ГОСТ Р 50571.5.52-2011; каталоги NED-Plagum 2025; Метод. рекомендации Роскоммунэнерго 2001"});
  ...
  srcLabel(line){ return {"auto":"IEC/Ямал (в воздухе) + ГТП ПУЭ (в земле)","YAMAL-LV":"Ямал СПГ табл.1 (IEC 60364-5-52, медь)","GTP-PUYE":"ГТП ПУЭ п.1.3.10 (проводники с пластмассовой изоляцией)","SN3x":"каталог NED-Plagum SN, 3-жильные","SN1x":"каталог NED-Plagum SN, 1-жильные","YAMAL-HV":"Ямал СПГ табл.2 (IEC 60502-2)","VL-AC":"ПУЭ табл.1.3.29 (неизолированные провода)","VL-SIP":"СИП-4 (ГТП)"}[line.src]||line.src; },
  docSection(x,i){ const self=this,n=x.n,line=n.line,S=self.settings,par=self.byId(n.par);
    ...
    h+="Iдоп.ф = Iкат·kθ·kсред = "+self.fmt(b.Icat,0)+" · "+self.fmt(b.kT,3)+" · "+self.fmt(b.kGrp,3)+" = "+self.fmt(b.Idp,0)+" А, где kθ = √((θраб−tср)/(θкат−tкат)) по формуле (1) Ямал СПГ: θкат="+b.theta0+" °С, tср="+b.tFact+" °С<br>";
    h+="группировка/среда: "+self.esc(b.groupNote||"")+(b.rho!=null?"; ρ="+b.rho+" К·м/Вт — ПУЭ табл.1.3.23":"")+"; (Ямал §7.7: kGA/kHA; ПУЭ 1.3.26 на число кабелей в траншее)<br>";
    h+="ΔU = "+(line.ph==="1"?"2·I·L·(r0·cosφ+x0·sinφ)/Uф":"√3·I·L·(r0·cosφ+x0·sinφ)/U")+" = "+self.fmt(b.dU.V,1)+" В = "+self.fmt(b.dU.pct,2)+" % ≤ "+(line.allowedPct!=null?line.allowedPct:"—")+" % (Ямал §7.9, ГОСТ 32144)<br>";
    ...
    if(line.prot&&line.prot.ust&&b.checks.mtz)h+="МТЗ (ПУЭ 3.1.11): Iуст="+line.prot.ust+" А ≤ "+b.checks.mtz.need+"·Iдоп.ф = "+self.fmt(b.checks.mtz.need*b.Idp,0)+" А; фактическая доля уставки "+self.fmt(b.checks.mtz.val*100,0)+"% — "+(b.checks.mtz.ok?"выполнено":"НАРУШЕНО")+"<br>";
    if(b.checks.sc)h+="термостойкость при КЗ (Ямал §7.10): "+self.esc(b.checks.sc.txt)+" — "+(b.checks.sc.ok?"выполнено":"нарушено")+"<br>";
    if(b.checks.start)h+="пусковой ΔU (Ямал §7.9, 15%): "+self.esc(b.checks.start.txt)+" — "+(b.checks.start.ok?"выполнено":"нарушено")+"<br>";
    if(line.motor&&line.motor.branch)h+="отпаечная к ЭД: Iдоп ≥ 1,25·Iн (ПУЭ 3.1.12/7.3.97, Ямал §7.2)<br>";
    h+="</p>"; return h; },
  ...
  buildDoc(full){ const self=this,S=self.settings,R=self.rep,now=new Date().toLocaleString("ru-RU");
    ...
    rows.forEach((x,i)=>{ const b=x.res.best,line=x.line,par=self.byId(x.n.par)||{};
      ...
      h+="<tr><td>"+(i+1)+"</td><td>"+self.esc(par.label+" → "+x.n.label)+"</td><td>"+(line.kind==="vl"?"ВЛ ":"КЛ ")+(+line.vclass===0.4?"0,4":line.vclass)+"</td><td>"+(line.ph==="1"?"1ф":"3ф")+"</td><td>"+self.fmt(effP,0)+"</td><td>"+(line.L||"—")+"</td><td>"+self.fmt(x.res.IrA,0)+"</td><td>"+(b?self.esc(self.markOf(line,b)):"—")+"</td><td>"+(b?self.fmt(b.Idp,0):"—")+"</td><td>"+(b&&b.dU?self.fmt(b.dU.pct,2):"—")+"</td><td>"+(b?(["PDT","dU","mtz","sc","minS","start"].every(k=>!b.checks[k]||b.checks[k].ok)?"годен":"с замеч."):  "не годен")+"</td></tr>"; });
    h+="</table>";
  ...
  buildXls(){ const self=this;
    ...
    self.collect().forEach(x=>{ const l=x.line,b=x.res.best; const par=self.byId(x.n.par)||{};
      ...
      h+="<tr><td>"+self.esc(par.label+" → "+x.n.label)+"</td><td>"+l.vclass+"</td><td>"+(l.kind==="vl"?"ВЛ":"КЛ")+"</td><td>"+(l.ph||"3")+"</td><td>"+eff+"</td><td>"+(l.cos||0.8)+"</td><td>"+(l.Ks||1)+"</td><td>"+(l.L||"")+"</td><td>"+self.esc((DATA.methods[l.method]||{}).label||"")+"</td><td>"+self.fmt(x.res.IrA,1)+"</td><td>"+(b?b.s:"—")+"</td><td>"+(b?self.esc(self.markOf(l,b)):"—")+"</td><td>"+(b?self.fmt(b.Idp,0):"")+"</td><td>"+(b&&b.dU?self.fmt(b.dU.pct,2):"")+"</td><td>"+(l.allowedPct||"")+"</td><td>"+((l.prot&&l.prot.ust)||"")+"</td><td>"+(b?["PDT","dU","mtz","sc","minS","start"].filter(k=>b.checks[k]).map(k=>k+(b.checks[k].ok?"+":"−")).join(" "):"")+"</td></tr>"; });
  ...
  buildCSV(){ const self=this; const q=v=>'\"'+String(v==null?"":v).replace(/"/g,'""')+"\"";
    ...
    self.collect().forEach(x=>{ const l=x.line,b=x.res.best;
      ...
      s+=[q(((self.byId(x.n.par)||{}).label||"")+" -> "+x.n.label),l.vclass,(l.kind==="vl"?"ВЛ":"КЛ"),(l.ph||"3"),((l.P!==""&&l.P!=null)?l.P:((self.effLoad(x.n)||{}).P||0)),(l.cos||""),(l.Ks||1),(l.L||0),q((DATA.methods[l.method]||{}).label||""),self.fmt(x.res.IrA,1),(b?b.s:""),q(b?self.markOf(l,b):""),(b?self.fmt(b.Idp,0):""),(b&&b.dU?self.fmt(b.dU.pct,2):""),(l.allowedPct||""),((l.prot&&l.prot.ust)||""),q(chk)].join(";")+"\n"; });
    return s; },
