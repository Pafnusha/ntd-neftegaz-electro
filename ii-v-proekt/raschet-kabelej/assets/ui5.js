...
Object.assign(App,{
  bindNTD(){ const self=this; const host=self.$("#ntdSections"); if(!host)return; host.innerHTML=""; const nav=self.$("#ntdNav"); nav.innerHTML="";
    ...
    mk("iec1","IEC 60364-5-52 · Таблица 1 (проектная методика) — длительно допустимые токи НН, 0,66/1 кВ, медь XLPE/аналог (90 °С; t возд 30 °С; IEC 60364-5-52)",c=>{
      c.append(self.$parse(TB(["<tr><th>сечение, мм²</th><th>в воздухе — лоток лестничный/перфорир. (B52.12-E/F)</th><th>в трубе (B52.5-B2)</th></tr>"]
        .concat(DATA.lvIec.map(r=>"<tr><td class='num'>"+r[0]+"</td><td class='num'>"+r[1]+"</td><td class='num'>"+(r[2]!=null?r[2]:"— (не нормируется &gt;300)")+"</td></tr>")))));});
    mk("iec2","IEC 60502-2 · Таблица 2 (проектная методика) — токи ВН брон. 3,6/6–18/30 кВ (Cu XLPE, IEC 60502-2 B6/B2) в воздухе",c=>{
      const keys=[...new Set(DATA.hvIecAir.c3.map(r=>r.s).concat(Object.keys(DATA.hvIecAir.c1trefoil).map(Number)))].sort((a,b)=>a-b);
      const rowsY=["<tr><th>сечение</th><th>3С (B.6)</th><th>1С тр-к (B.2)</th></tr>"];
      keys.forEach(s2=>{ const c3=DATA.hvIecAir.c3.find(r=>r.s===s2), c1=DATA.hvIecAir.c1trefoil[s2];
        rowsY.push("<tr><td class='num'>"+s2+"</td><td class='num'>"+(c3?c3.i:"—")+"</td><td class='num'>"+(c1!=null?c1:"—")+"</td></tr>"); });
      c.append(self.$parse(TB(rowsY)));});
    mk("iec3","kAT (формула (1) IEC / проектной методики) — коэффициенты по температуре воздуха (формула (1): k=√((K−T)/(K−t)); K=90 XLPE/EPR · 70 PVC · 65 бумага; t=30 — в воздухе)",c=>{
    ...
    mk("iec4","§7.7 — общие коэффициенты снижения (типовые маршруты) снижения (типовые маршруты проекта)",c=>{
      let h="<table class='tbl'><tr><th>назначение</th><th>Σ k</th><th>состав</th></tr>";
      DATA.projOverall.forEach(r=>h+="<tr><td>"+self.esc(r.name)+"</td><td class='num'>"+r.f+"</td><td class='muted'>"+self.esc(r.d)+"</td></tr>"); c.append(self.$parse(h+"</table>"));});
    mk("iec5","§7.9 — допустимые потери напряжения напряжения; ГОСТ 32144-2013",c=>{
      let h="<table class='tbl'><tr><th>цепь/назначение</th><th>ПДУ, %</th></tr>";
      DATA.duProj.forEach(r=>h+="<tr><td>"+self.esc(r[0])+"</td><td class='num'>"+r[1]+"</td></tr>");
    ...
    mk("iec6","§7.10 — выдерживаемые токи КЗ (типовые уставки: таблица времени/тока; адиабатное S=I·√t/K)",c=>{
    ...
    mk("klist","Коэффициент K материала (адиабатное уравнение, §7.10 / IEC 60364-4-43)",c=>{
    ...
    mk("lvgt","ГТП по ПУЭ п.1.3.10 (пластмасса, θ жил 65 °С, возд. 25/земля 15) — токи 0,66–3 кВ; движок пересчитывает на 90 °С формулой (1)",c=>{
      ...
      c.append(self.$parse(h));});
    mk("mtz","МТЗ и защита от перегрузки: ПУЭ 3.1.10–3.1.12, 7.3.97",c=>{
    ...
    mk("jed","Экономическая плотность тока (ПУЭ табл. 1.3.36) — справочно, критерий не включён в выбор",c=>{
      ...
      c.append(self.$parse(h+"</table></div>"));});
    mk("fire","Невозгораемость кабелей — ГОСТ 31565 / исполнения нг",c=>{
      let h="<p>Проверка в расчёте: марка должна содержать признак нераспространения горения <b>нг</b>, <b>нг(А)-LS</b>, <b>нг(А)-HF</b>, FRLS/FRHF и т.п. (ГОСТ 31565-2012). База марок — каталог NED-Plagum.</p><table class='tbl'><tr><th>код</th><th>описание</th></tr>";
      (DATA.snBuilds||[]).forEach(b=>h+="<tr><td>"+self.esc(b.code)+"</td><td>"+self.esc(b.name)+"</td></tr>");
      (DATA.lvBrands||[]).forEach(b=>h+="<tr><td>"+self.esc(b.code)+"</td><td>"+self.esc((b.shells||"")+" "+(b.armor||""))+"</td></tr>");
      c.append(self.$parse(h+"</table>"));});
  ...
  formulasHelpText(){ return "<div class='formula-box'>"+
    "1) Iр=Kс·P·10³/(√3·U·cosφ) [3ф]; Iр=Kс·P·10³/(Uф·cosφ) [1ф] — основы МЭК / проектная методика §7.5.<br>"+
    "2) Iдоп.факт = Iкат · √((θраб−tср)/(θкат−tкат)) · kρ·kгрупп · kд.пол — формула (1) IEC + ПУЭ табл.1.3.26 (в траншее), каталог NED (в воздухе/каналах), ПУЭ 1.3.23 (ρ базы 1,2).<br>"+
    "3) ΔU(3ф)=100·√3·I·L·(r0·cosφ+x0·sinφ)/(U·1000) %; ΔU(1ф)=100·2·I·L·(r0 cosφ+x0 sinφ)/(Uф·1000) — формула (2), IEC 60364.<br>"+
